...
public class Transformer {
	 ...
	 *            a MARCDataField
	 * @param selection
	 *            a string of codes such as "$a $d", optionally preceded by a
	 *            dash, "-$a $d", which denotes exclusion.
	 * @return The subfield values whose code is contained in the selection, in
	 *         the order they are found in the field. If the selection starts
	 *         with a dash then, all subfields are selected except for those in
	 *         the selection.
	 */
	 ...
	 *            dash, "-$a $d", which denotes exclusion.
	 * @param separator
	 *            the separator string between subfields
	 * @return The subfield values whose code is contained in the selection, in
	 *         the order they are found in the field. If the selection starts
	 *         with a dash then, all subfields are selected except for those in
	 *         the selection.
	 ...
	 *            a MARCDataField
	 * @param selection
	 *            array of prefix of codes such as {"$a","$d"}.
	...
	/**
	 * @param subfield
	 *            a MARC subfield (e.g., "$a Mariano José de Larra").
	 * @param pattern
	 *            the content-pattern (e.g., "$a:de Larra", "$a=Larra",
	 *            "$a+Larra" or "$a-Larra")
	 ...
	 * 
	 * @param filed
	 *            the MARCDataField to be compared with the content-patterns
	 * @param patterns
	 *            the list of content-patterns
	 */
	 ...
	 *            a MARC record.
	 * @param tags
	 *            the selected field tags.
	 * @param patterns
	 *            array of content requirements of the form "$a:de Larra" (or
	 *            contains), "$a=Larra" (or exact), "$a+Larra" (and contains)
	...
	/**
	 * @param list
	 *            a repeatable field such as {"$a name", "$d dates", "$$",
	 *            "$a anothername"}.
	 * @param selection
	 *            array of prefix of codes such as {"$a","$d"}.
	 * @param patterns
	 *            array of content requirements of the form "$a:de Larra"
	 *            (contains), "$a=Larra" (exact) or "$a-Larra"(exclude).
	 * @return for every field matching the all patterns, the subfield values
	 *         whose prefix code is contained in the selection, in the order
	 *         they are selected.
	 * 
	 *         public static List<String> apply(List<String> list, String[]
	 *         selection, String[] patterns) { List<String> res = new
	 *         ArrayList<String>(); String selected;
	 * 
	 *         while (list.size() > 0) { int last = list.indexOf(separator); if
	 *         (last < 0) { if (matchesAllPatterns(list, patterns)) { selected =
	 *         toString(list, selection); if (selected.length() > 0) {
	 *         res.add(selected); } } list = list.subList(0, 0); } else {
	 *         List<String> sublist = list.subList(0, last); if
	 *         (matchesAllPatterns(sublist, patterns)) { selected =
	 *         toString(sublist, selection); if (selected.length() > 0) {
	 *         res.add(selected); } } list = list.subList(last + 1,
